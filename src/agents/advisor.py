...
5명의 투자자 관점이 상충할 경우, 답변 마지막에 반드시:
**[종합 결론]** 단락을 추가하여 상충하는 관점 중 현재 시장 데이터 기준으로
가장 적합한 단일 행동 방침을 제시합니다.

---

## 고위험 키워드 경고 규칙

사용자 질문에 아래 키워드가 포함되면 반드시 경고 박스를 먼저 출력합니다:
- "올인", "전액", "몰빵", "레버리지", "신용", "빚투", "대출"

경고 형식:
> ⚠️ **고위험 전략 감지**: [키워드] 전략은 원금 전액 손실 위험이 있습니다.
> 투자 원칙상 단일 자산 집중·레버리지 투자는 권장하지 않습니다.
...
_HIGH_RISK_KEYWORDS = ["올인", "전액", "몰빵", "레버리지", "신용", "빚투", "대출투자"]
...
def _detect_high_risk(text: str) -> list[str]:
    return [k for k in _HIGH_RISK_KEYWORDS if k in text]
...
def chat_stream(user_message: str, history: list[dict], portfolio: dict | None = None):
    ...
    # High-risk keyword warning prepended to context
    risky = _detect_high_risk(user_message)
    if risky:
        context_parts.append(
            f"[시스템 경고] 사용자 질문에 고위험 키워드 감지: {', '.join(risky)}\n"
            "반드시 경고 박스를 답변 첫 줄에 출력하세요."
        )
    ...
    for ticker in tickers:
        try:
            info = get_stock_info(ticker)
            week_low = info.get("week_52_low") or 0
            week_high = info.get("week_52_high") or 0
            context_parts.append(
    ...
    system_content = [
        {
            "type": "text",
            "text": SYSTEM_PROMPT.format(philosophy=philosophy),
    ...
    try:
        ...
        # Fallback without prompt caching if beta not available
        with client.messages.stream(
            model="claude-sonnet-4-6",
            max_tokens=2048,
            system=SYSTEM_PROMPT.format(philosophy=philosophy),
            messages=messages,
        ) as stream:
            for text in stream.text_stream:
                yield text
